...
def train(model, device, train_loader, valid_loader, epochs, lf, optimizer):
    ...
    for epoch in range(1, epochs + 1):
        ...
        for i, data in enumerate(train_loader):
            ...
            pbar.set_postfix(
                    {"loss": loss.item(), "Predicted Class": predicted_class, "Actual Class": data[1]})
...
def main(t_csv, v_csv, b_size, t_device, lr, eps_num, ls_function, d_set):
    ...
    train_data = dataPreprocess.AudioDataset(t_csv, label_list, "train", d_set)
    vali_data = dataPreprocess.AudioDataset(v_csv, label_list, "validation", d_set)
    ...
    resnet_model = model.resnet18(1, 6)  # 1 channel, 6 classes
